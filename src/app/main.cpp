// scshr — native Windows viewer for Apple Screen Sharing High Performance mode.
//
//   Winsock IOCP → SRTP → RTP/NAL → FFmpeg D3D11VA → decoder texture slice → D3D11 pixel shader → DXGI flip-model Present
//
// Render loop (main thread): wait for the swapchain's frame-latency waitable object, THEN take the newest
...
void usage() {
    std::puts("scshr — native client for Apple Screen Sharing High Performance mode\n"
              "  scshr init [--server-code SCST1:...]   scshr status   scshr tunnel uninstall [--reset-identity]\n"
...
std::string read_password(const Args& a) {
    if (!a.password_stdin) {
...
int main(int argc, char** argv) {
    ...
    if (!replay && a.user.empty()) { usage(); return 2; }
    if (a.direct && !replay && a.host.empty()) { std::fprintf(stderr, "--direct requires --host\n"); return 2; }
    // Fail closed: production sessions only ever talk to the paired tunnel address.
    try { a.host = app::resolve_session_host(a.host, a.direct || replay); }
    catch (const std::exception& e) { std::fprintf(stderr, "scshr: %s\n", e.what()); return 1; }
    if (!replay) a.password = read_password(a);
    timeBeginPeriod(1);
    ...
    if (a.advertise != "auto") {
        ...
        else { std::fprintf(stderr, "bad --advertise %s\n", a.advertise.c_str()); return 2; }
    ...
    std::unique_ptr<app::KeyboardHook> hook;
    struct View {
    ...
    primary.window = std::make_unique<app::Window>(L"scshr — connecting…", win_w, win_h, make_events(&primary));
    app::Window* window = primary.window.get();
    int primary_id = 0;

    RendererConfig rc; rc.adapter_override = a.adapter; rc.present_mode = a.present;
    try { renderer.init(window->hwnd(), rc); } catch (const std::exception& e) { LOG_ERROR("app", "renderer init failed: %s", e.what()); return 1; }
    ...
    session->read_local_clipboard = [] { return app::clipboard_read_text(); };
    session->write_local_clipboard = [](const std::string& s) { app::clipboard_write_text(s); };
    ...
    conn.join();
    if (closed) { session->close(); return 0; }
    if (conn_state == 2) { LOG_ERROR("app", "connect failed: %s", conn_error.c_str()); MessageBoxA(window->hwnd(), conn_error.c_str(), "scshr: connect failed", MB_ICONERROR); return 1; }
    ...
    if (cw0 == 0 || ch0 == 0) {
        ...
        if (cw0 == 0 || ch0 == 0) { LOG_ERROR("app", "no decoded frame within 10 s"); session->close(); return 1; }
    }
    ...
    window->set_title(std::wstring(L"scshr — ") + std::wstring(a.host.begin(), a.host.end()));
    if (!dynamic) window->lock_aspect(sc.advertise.width, sc.advertise.height);
    if (a.grab) { hook = std::make_unique<app::KeyboardHook>(window->hwnd(), [&](bool d, uint32_t ks) { session->key_event(d, ks); }); if (window->focused()) hook->enable(); }
    ...
    while (!closed && now_ns() < deadline) {
        ...
        if (!session->is_connected()) { LOG_ERROR("app", "connection lost — closing viewer"); break; }
            ...
            if (!rects.empty()) {
                if (rects.size() > 1) {
                    ...
                    for (size_t i = 1; i < rects.size(); ++i) {
                        ...
                        v->window = std::make_unique<app::Window>(L"scshr — monitor " + std::to_wstring(i + 1), base, std::max(1, base * r.h / std::max(1, r.w)), make_events(v.get()));
